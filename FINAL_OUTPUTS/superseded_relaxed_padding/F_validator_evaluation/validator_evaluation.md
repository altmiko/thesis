...
| Dataset | Category | Layer | Rules |
|---|---|---|---|
| CICIDS2017 | SCHEMA | general flow consistency | 133 |
| CICIDS2017 | EXTRACTOR | general flow consistency | 7 |
| CICIDS2017 | PROTOCOL | general flow consistency | 79 |
| CICIDS2017 | MINED | dataset-specific (train-mined) | 16 |
| CICIDS2018 | SCHEMA | general flow consistency | 133 |
| CICIDS2018 | EXTRACTOR | general flow consistency | 7 |
| CICIDS2018 | PROTOCOL | general flow consistency | 77 |
| CICIDS2018 | MINED | dataset-specific (train-mined) | 10 |
...
acceptance on CICIDS2017 and 0.064–0.067 pp on CICIDS2018. The same mined layer also flags most
invalid adversarial flows in Exp A–E. It flags 84–100% of invalid CAPGD/C-PGD examples (which
also fail EXTRACTOR rules) and ≥ 98.5% of invalid PGD/C&W examples (which fail every
category). It is the only category that rejects PrimAttack's invalid CICIDS2018 examples, all
through `MINED_0001`. The rule that falsely rejects about 0.06% of genuine CICIDS2018 flows
therefore accounts for PrimAttack's entire CICIDS2018 validity gap. Its false-rejection rate
is a caveat when reading that gap: the rule holds in 99.94% of genuine held-out flows, not in all
of them.

**Reading (Contribution 4).** The validator combines general flow-consistency constraints
(133 SCHEMA, 7 EXTRACTOR and 77–79 PROTOCOL rules) with automatically mined, train-only
dataset-specific invariants (16 rules on CICIDS2017, 10 on CICIDS2018). On data it has never
seen, its false-rejection rate is ≤ 0.07%, so "invalid" verdicts on adversarial flows are
not an artefact of an over-strict validator. Acceptance is a necessary structural check, not a
proof of realizability or malicious functionality: a flow can pass every rule and still not be
producible by any real packet trace.
