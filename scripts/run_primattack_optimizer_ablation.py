...
``outputs/primattack_optimizer_ablation/<dataset>``). An existing output directory is never
overwritten; pass ``--resume`` to finish it.
...
from run_full_adversarial_eval import (  # noqa: E402
    BENIGN_ID, BUDGET_LABEL, DATASET_DEFAULTS, file_sha256, victim_checkpoint,
)
...
def run_method(method, cfg, args_tuple, *, seed, gate, budget, objective):
    if method == "hybrid":
        return optimize_primitive_candidates(
            *args_tuple, steps=cfg["steps"], learning_rate=cfg["learning_rate"], seed=seed,
            restarts=cfg["restarts"], validity_fn=gate, eval_budget=budget, objective=objective)
    if method == "pgd":
        return optimize_primitive_pgd(
            *args_tuple, steps=cfg["steps"], step_size=cfg["step_size"],
            ...
            validity_fn=gate, eval_budget=budget, objective=objective)
    if method == "cw":
...
def main() -> None:
    ...
    ap.add_argument("--objective", choices=("targeted", "untargeted"), default="targeted",
                    help="targeted: realized flow -> Benign; untargeted: realized flow leaves "
                         "its true source class")
    ...
    ap.add_argument("--cw-lr", type=float, default=0.5,
                    help="validation-selected (primattack_optimizer_ablation.md §3.4)")
    ...
    for vname in victims:
        ...
        victim = load_category_victim(ckpt, adapter=adapter, expected_model_type=arch,
                                      device=device)
        victim_info[vname] = {"checkpoint": str(ckpt), "checkpoint_sha256": file_sha256(ckpt),
                              "arch": arch, "train_seed": train_seed,
        ...
        for cname in classes:
            ...
            rows[(vname, cname)] = {
                ...
                "clean_raw_sha256": hashlib.sha256(
                    np.ascontiguousarray(raw.cpu().numpy()).tobytes()).hexdigest(),
    ...
    config = {
        ...
        "victims": {v: {k: i[k] for k in ("checkpoint", "checkpoint_sha256", "arch",
                                          "train_seed", "attack_seeds")}
                    for v, i in victim_info.items()},
        "split": split, "selection_from": str(sel_path) if sel_path else f"fresh ({split})",
        "selection_sha256": file_sha256(sel_path) if sel_path else None,
            ...
            for budget in budgets:
                ...
                for seed in info["attack_seeds"]:
                    for method in methods:
                        ...
                        masks, _, clean_pred, adv_pred = evaluate_cell(
                        ...
                        untargeted = np_(adv_pred != cid)
                        hit = targeted if args.objective == "targeted" else untargeted
                        ...
                        final_success = hit & valid
                        consistent = final_success == success
                        first_t = np_(res.first_objective_hit_evaluation)
                        changed = adv != raw
                        outside = changed[:, ~support_mask].sum(1)
                        if bool((outside != 0).any()):
                            raise AssertionError(
                                f"PrimAttack changed a feature outside its joint support {key}")
                        failure = np.where(
                        ...
                        np.savez_compressed(
                            ...
                            objective=args.objective, raw_success=hit,
                            validator_pass=valid, valid_success=final_success,
                            targeted_success=targeted, untargeted_success=untargeted,
                            ...
                            objective_margin=np_(res.objective_margin),
                            n_features_modified=np_(changed.sum(1)).astype(np.int64),
                            n_modified_outside_primattack_mask=np_(outside).astype(np.int64),
                            adv_raw=np_(adv).astype(np.float32),
                            clean_raw_sha256=R["clean_raw_sha256"],
                            ...
                            first_objective_hit_evaluation=first_t,
                        ...
                        cell = {
                            ...
                            "raw_successes": int(hit.sum()),
                            "asr_raw": float(hit.mean()),
                            "validator_pass_rate": float(valid.mean()),
                            "max_modified_outside_primattack_mask": int(outside.max().item()),
