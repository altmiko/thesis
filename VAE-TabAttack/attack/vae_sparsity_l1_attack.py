...
class VAESparsityL1Attack:
    def __init__(self, 
                 ml_model: nn.Module,
                 dataset: str, 
                 model: str,
                 lambda_: float = 0.5, 
                 lambda_sparsity: float = 0.1,  # New sparsity weight parameter
                 optimizer: str = "adam", 
                 lr: float = 1e-2,
                 max_iter: int = 1000,
                 kappa: float = 0.0,
        ...
        self._lambda_sparsity = lambda_sparsity  # Weight for sparsity term
        self._kappa = kappa
        ...
        self._attack = "sparsity_l1"  # Updated attack name

    def _compute_sparsity_loss(self, original_instance, adv_example):
        """
        Compute sparsity loss using L1 norm calculation.
        ...
        Returns:
            Sparsity loss (count of features changed beyond threshold)
        """
        # calculate L1 distance between original and adversarial example
        sparsity_loss = torch.norm(original_instance - adv_example, p=1)
    ...
    def get_adversarial_examples(
        ...
        folder = os.path.join(folder, f"_lambda_{self._lambda}_sparsity_{self._lambda_sparsity}_lr_{self._lr}_max_iter_{self._max_iter}")
            ...
            for idx, query_instances in enumerate(test_loader):
                ...
                for i in range(query_instances.size(0)):  # Iterate over instances in the batch
                    ...
                    for _ in range(self._max_iter):  # Iterate over optimization steps
                        ...
                        # Compute loss
                        loss, distance = self._compute_loss(z_single, z_init_single, adv_example, original_class_single, original_instance_single)
    ...
    def _compute_loss(self, z, z_init, adv_example, original_class, original_instance):
        ...
        # Sparsity term - measure the number of perturbed features in original space
        sparsity_loss = self._compute_sparsity_loss(original_instance, adv_example)

        # Combine losses
        loss = perturbation_loss + self._lambda * f + self._lambda_sparsity * sparsity_loss
