...
SEARCH_GRID = {
    "radius":    [0.5, 1.0, 2.0, 3.0, 5.0],   # was [2,3,5,8,12,16]
...
class MixedInputVAE(nn.Module):
    def __init__(self, input_dim, hidden_dims, latent_dim, continuous_idx, binary_idx):
        ...
        self.binary_head = nn.Sequential(nn.Linear(in_dim, len(binary_idx)),
                                         nn.Sigmoid())
...
def compute_safe_feature_mask(X_train: np.ndarray,
                               threshold: float = CORRELATION_THRESHOLD) -> torch.Tensor:
    """
    ...
    safe_cont_cols = [
        ...
        if (corr[col].drop(index=col) < threshold).all()
    ]
    ...
    print(f"[FeatureMask] Safe features: {n_safe}/{n_cont} continuous  "
          f"(threshold={threshold})")
    if n_safe == 0:
        print("[FeatureMask] WARNING: no safe features found — "
...
def check_validity(X_adv: np.ndarray,
                   X_orig: np.ndarray,
                   cont_min: np.ndarray,
                   cont_max: np.ndarray) -> np.ndarray:
    """
    ...
    cont_valid = (
        (cont_adv >= cont_min[CONTINUOUS_IDX] - 1e-4).all(axis=1) &
        (cont_adv <= cont_max[CONTINUOUS_IDX] + 1e-4).all(axis=1)
    )
    bin_valid = np.all(
        (bin_adv == 0) | (bin_adv == 1), axis=1
    )
...
def constraint_loss(x_adv: torch.Tensor,
                    cont_min_t: torch.Tensor,
                    cont_max_t: torch.Tensor) -> torch.Tensor:
    """
...
def latent_attack_cnnlstm(x_batch, original_class_idx, vae, cnnlstm,
                           radius, max_iter, lambda_cw, lr,
                           ...
                           use_ste=True):    # NEW: use straight-through estimator
    """
    ...
    orig_labels = torch.full((B,), original_class_idx,
    ...
    for _ in range(max_iter):
        ...
        z_proj = torch.where(z_norm > conservative_radius, 
                             delta * conservative_radius / z_norm, delta)
        z_adv  = mu + z_proj
    ...
    with torch.no_grad():
        ...
        z_adv  = mu + z_proj
        cont_out, bin_out = vae.decode(z_adv)
        x_decoded = decode_to_full(cont_out, bin_out, use_ste=False)  # Hard threshold for eval
...
def latent_attack_lgbm(x_batch, original_class_idx, vae, lgbm_model,
                        radius, max_iter, lr,
                        safe_mask,         # NEW
    ...
    for step in range(max_iter):
        ...
        z_cand    = np.where(diff_norm > conservative_radius,
...
def grid_search(class_name, class_idx, X_probe, vae, cnnlstm,
                safe_mask, cont_min_t, cont_max_t, use_ste=True):
    ...
    for i, combo in enumerate(combos):
        ...
        _, suc = latent_attack_cnnlstm(
            x_probe_t, class_idx, vae, cnnlstm,
            radius     = cfg["radius"],
        ...
        if (i + 1) % 20 == 0 or (i + 1) == total:
            print(f"    [{i+1}/{total}] r={cfg['radius']} λ={cfg['lambda_cw']} "
...
def evaluate_asr_multiseed(X_cls, class_idx, vae, cnnlstm, lgbm_model,
                            best_cfg, safe_mask, cont_min_t, cont_max_t,
                            cont_min_np, cont_max_np, seeds=EVAL_SEEDS,
                            use_ste=True):
    """
    ...
    for seed in seeds:
        ...
        for start in range(0, len(X_cls), BATCH_SIZE):
            ...
            adv, suc = latent_attack_cnnlstm(
                xb, class_idx, vae, cnnlstm,
                radius=radius, max_iter=max_iter,
                lambda_cw=lambda_cw, lr=lr,
                safe_mask=safe_mask,
                cont_min_t=cont_min_t, cont_max_t=cont_max_t,
                use_ste=use_ste,
            )
        ...
        valid_c = check_validity(adv_c, X_cls,
                                 cont_min_np, cont_max_np)
        ...
        for start in range(0, len(X_cls), BATCH_SIZE):
            ...
            adv, suc = latent_attack_lgbm(
                xb, class_idx, vae, lgbm_model,
                radius=radius, max_iter=max_iter, lr=lr,
                safe_mask=safe_mask,
                cont_min_np=cont_min_np, cont_max_np=cont_max_np,
        ...
        valid_l = check_validity(adv_l, X_cls,
                                 cont_min_np, cont_max_np)
...
def main(use_train_bounds=False, use_ste=True):
    ...
    if use_train_bounds:
        try:
            ...
            print(f"[Bounds] WARNING: X_train.npy not found, falling back to test set bounds")
    ...
    cont_min_t  = torch.tensor(cont_min_np[CONTINUOUS_IDX],
                               dtype=torch.float32, device=DEVICE)
    cont_max_t  = torch.tensor(cont_max_np[CONTINUOUS_IDX],
                               dtype=torch.float32, device=DEVICE)
    print(f"Continuous bounds computed from {'train' if use_train_bounds else 'test'} set.")
    ...
    for class_name in ATTACK_CLASSES:
        ...
        print(f"\n{'='*60}")
        print(f"Class: {class_name}  |  pool={n_pool}  "
        ...
        if class_name in SEARCH_CLASSES:
            ...
            best_cfg, probe_asr = grid_search(
                class_name, class_idx, X_probe, vae, cnnlstm,
                safe_mask, cont_min_t, cont_max_t, use_ste=use_ste,
            )
        else:
        ...
        stats = evaluate_asr_multiseed(
            X_cls, class_idx, vae, cnnlstm, lgbm,
            best_cfg, safe_mask,
            cont_min_t, cont_max_t,
            cont_min_np, cont_max_np,
            use_ste=use_ste,
        )
