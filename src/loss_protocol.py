...
def compute_protocol_loss(x_adv, scaler_mean, scaler_scale):
    # x_adv: (B, 37)
    ...
    loss += torch.relu(binary[:, TCP] + binary[:, UDP] - 1.0).sum()
    
    # 2. ARP -> not TCP, UDP, ICMP
    arp = binary[:, ARP]
    loss += (arp * binary[:, TCP]).sum()
    loss += (arp * binary[:, UDP]).sum()
    loss += (arp * binary[:, ICMP]).sum()
    
    # 3. ICMP -> not TCP, UDP
    icmp = binary[:, ICMP]
    loss += (icmp * binary[:, TCP]).sum()
    loss += (icmp * binary[:, UDP]).sum()
    ...
    loss += (app_tcp * torch.relu(1.0 - binary[:, TCP])).sum()
    
    # 5. DNS -> TCP or UDP
    dns = binary[:, DNS]
    transport = torch.clamp(binary[:, TCP] + binary[:, UDP], max=1.0)
    loss += (dns * torch.relu(1.0 - transport)).sum()
    
    # 6. DHCP -> UDP
    dhcp = binary[:, DHCP]
    loss += (dhcp * torch.relu(1.0 - binary[:, UDP])).sum()
    ...
    loss += (flags_active_bool * torch.relu(1.0 - binary[:, TCP])).sum()
    ...
    loss += torch.relu(raw_min - raw_avg).sum()
    loss += torch.relu(raw_avg - raw_max).sum()
    
    # 10. Non-neg counts (in scaled space < -3)
    loss += torch.relu(-3.0 - cont[:, ACK_CNT]).sum()
    loss += torch.relu(-3.0 - cont[:, SYN_CNT]).sum()
    loss += torch.relu(-3.0 - cont[:, FIN_CNT]).sum()
    loss += torch.relu(-3.0 - cont[:, RST_CNT]).sum()
