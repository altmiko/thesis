...
| `src/attack/primitive_optimizer.py` | `RealizedSearch` (shared projection → quantized realization → victim + `hybrid_valid` success gate → per-flow incumbent and evaluation counts) and three optimizers on it: `optimize_primitive_candidates` (Hybrid: exact integer-padding enumeration + adaptive projected refinement; the default), `optimize_primitive_pgd`, `optimize_primitive_cw` (ablation baselines) | LIVE (core) |
...
| `scripts/run_primattack_optimizer_ablation.py` | Controlled PrimAttack runs (Hybrid / Prim-PGD / Prim-C&W) on a frozen `selection.json` at a matched per-flow evaluation budget; `--objective targeted` or `untargeted`; `--modes joint,timing-only,padding-only`; per-row npz incl. final adversarial flow, capabilities and per-row search mode; `--split val` for tuning → `outputs/primattack_optimizer_ablation/<dataset>/` by default | LIVE |
| `scripts/run_final_suite.py` | FINAL thesis suite driver (`master_experiments.md`, protocol `FINAL_OUTPUTS/00_PROTOCOL.md`): baselines (PGD, C&W, CAPGD/C-PGD-PrimSupport, native CAPGD †), PrimAttack optimizer selection, budgets, untargeted, primitive-mode ablation; seeds 42/2024/2026, both datasets → `FINAL_OUTPUTS/runs/` | LIVE |
| `scripts/analyze_final_suite.py` | FINAL suite audit (pairing, validator/prediction recheck) + tables, Cochran's Q / McNemar / Holm, plots, reports for Exp A–F, capability-fix analyses (`A_primary_baseline_comparison/capability_fix/`) → `FINAL_OUTPUTS/` | LIVE |
| `scripts/report_primattack_capability_fix.py` | Root audit report `primattack_empty_packet_fix_report.md` (empty-forward-packet capability fix, amendment A2) from `FINAL_OUTPUTS` + regression tests | LIVE |
